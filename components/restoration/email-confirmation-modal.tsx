...
interface EmailConfirmationModalProps {
  isOpen: boolean
  onClose: () => void
  onConfirmDownload: (email: string, targetSrc: string | string[]) => void // Modified to accept string or string[]
  downloadTargetSrc: string | null // For single photo download
  isDownloadingAll: boolean // New prop to indicate if all photos are being downloaded
  allPhotoSrcs: string[] // New prop for all photo sources when downloading all
}
      ...
      <DialogContent className="sm:max-w-[425px] bg-brand-background text-brand-text p-6 rounded-lg shadow-xl">
        <DialogHeader>
          ...
          <DialogDescription className="text-brand-text/70">
            Please enter your email address to receive the restored images.
          </DialogDescription>
        </DialogHeader>
            ...
            <>
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  className="col-span-3 bg-white border-brand-text/20 focus:border-brand-cta focus:ring-brand-cta"
                />
